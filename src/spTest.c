...
char *sbrk(int increment) {
    ...
    if (base == NULL) {
        ...
        base = (char *)system_info.lpMinimumApplicationAddress;
        heap_end = base;
    }
    ...
    if (increment > 0) {
        void *request = VirtualAlloc(heap_end, increment, MEM_COMMIT, PAGE_READWRITE);
        if (request == NULL) {
        ...
        // For negative increment, we don't actually free memory,
        // we just move the heap_end pointer
        if (!VirtualFree(new_heap_end, -increment, MEM_DECOMMIT)) {
            return (char *)-1;
        }
    }
...
double Time() {
    struct time {
        long user, system, childuser, childsystem;
    } time;
    (void)times(&time);
...
ReadMatrixFromFile() {
    ...
    if (!strncmp(Message, "Starting", 8)) { /* Test for complex matrix. */
